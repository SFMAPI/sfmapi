...
$env:SFMAPI_DB_URL = "sqlite+aiosqlite:///./test_sqlite.db"
& uv run pytest -q -m "not needs_pycolmap and not needs_postgres" @ExtraArgs
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

if ($env:SFMAPI_DB_URL_PG) {
    Write-Host "=== Postgres ===" -ForegroundColor Cyan
    $env:SFMAPI_DB_URL = $env:SFMAPI_DB_URL_PG
    & uv run pytest -q -m "not needs_pycolmap" @ExtraArgs
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
} else {
    Write-Host "(skipping Postgres run; set SFMAPI_DB_URL_PG to enable)"
}
