...
class SfmApiError(Exception):
    ...
    ``app/main.py::sfmapi_error_handler`` turns these into
    ...
    typed surface in :class:`~app.schemas.api.common.ProblemResponse`.
    """
...
class TenantViolationError(SfmApiError):
    ...
    Raised by :func:`app.core.tenancy.current_tenant` when the
...
class CapabilityUnavailableError(SfmApiError):
    ...
    the canonical feature name (see :mod:`app.core.capabilities`) so
